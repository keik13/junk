 ...
 * С помощью него возможно писать небольшие тесты и следить за корректностью своей программы (в обычной ситуации предпочтительно для этих целей использовать библиотеки
 ...
 */
public class Step6_4_14 {
    public static void main(String[] args) {
        // Random variables
        String randomFrom = "govno"; // Некоторая случайная строка. Можете выбрать ее самостоятельно.
        String randomTo = "mocha";  // Некоторая случайная строка. Можете выбрать ее самостоятельно.
        int randomSalary = 100;  // Некоторое случайное целое положительное число. Можете выбрать его самостоятельно.
...
//   где по получателю можно получить список зарплат, которые были ему отправлены.
    ...
    public static class MailService<T> implements Consumer<Sendable<T>> {
        private Map<String, List<T>> mailBox = new MapDefaultValue<>();

        @Override
        public void accept(Sendable<T> message) {
            List<T> contents = mailBox.get(message.getTo());
            if (!contents.isEmpty()) {
                contents.add(message.getContent());
            } else {
                List<T> newContents = new ArrayList<>();
                newContents.add(message.getContent());
                mailBox.put(message.getTo(), newContents);
            }
        ...
        public class MapDefaultValue<K, V> extends java.util.HashMap<K, V> {
            @Override
            public V get(Object key) {
                return super.getOrDefault(key, (V) Collections.<V>emptyList());
            }
        }
    }
}
